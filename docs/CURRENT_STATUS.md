...
- Message rows resolve sender identity from `state.sendersById[message.senderId]` (Room-backed). Each bubble shows circular avatar (Coil via `ProfileImagePublicUrlFactory` / public `profile-images` URL, or placeholder), username (or stable "Unknown" fallback), and existing timestamp. Outgoing and incoming text/media rows share this path. No per-row network profile fetch; Compose does not call Supabase.
