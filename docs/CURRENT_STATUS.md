...
Required Android implementation and manual runtime verification for the assignment are complete. Bonus #1 phase 1 (visual foundation, Profile Setup polish, Chat shell, message bubbles/states) is implemented on this branch and awaiting manual visual verification. Remaining required work for submission docs may still live on other branches; do not mix with FCM/voice/tests here.
...
- `:core:ui`: shared Compose UI utilities (`clearFocusOnTap()`, modest `ChatUiTokens` for spacing/bubble/avatar sizes).
...
- Messages keep only `senderId`; sender username/avatar stay normalized on Room `users` / domain `User`.
...
- `DefaultUserRepository` is local-first for lookup, caches remote users, and upserts remote then Room. `observeUsers` is Room-only.
...
- `ChatScreen` has Material 3 app bar on `surface`, quieter `surfaceContainerLowest` conversation canvas, empty state, polished message bubbles (asymmetric corners via `ChatUiTokens`, incoming sender name only, shared avatars), composer with compact attachment preview strip (ordered thumbnails, video badge/placeholder, per-item remove with a 48dp touch target, `n / 10` count), snackbar, Light/Dark previews, enabled attachment affordance, and `clearFocusOnTap()` on chat content (not on the composer). The composer uses a Material 3 `Button` for Send. The TextField is not disabled or made read-only during send, so the IME is not restarted. Send routes text-only through `sendTextMessage` and media (+ optional text) through `sendMediaMessage`; successful media handoff clears selection and text. Sending a new outgoing optimistic `SENDING` message animates the reversed list to index `0`. A live incoming Room message also scrolls to index `0` only when the list is already at/near the newest end (`firstVisibleItemIndex <= 1`); reading older history is not yanked. Older-page Room updates do not scroll.
- Message rows resolve sender identity from `state.sendersById[message.senderId]` (Room-backed). Incoming bubbles show circular avatar, username (or "Unknown"), and timestamp; outgoing bubbles show avatar, timestamp, and send state without a redundant own-username label. Coil loads avatars via `ProfileImagePublicUrlFactory`. No per-row network profile fetch; Compose does not call Supabase.
- Profile Setup uses clearer hierarchy (welcome copy, bordered avatar + primary badge affordance, medium-shaped fields, prominent Continue). Save/validation/Photo Picker behavior is unchanged; keyboard still uses Scaffold `safeDrawing` only.
...
- Bonus #1 phase 1 (static visual polish for foundation / Profile Setup / Chat shell / bubbles) is implemented; later polish (entry animations, composer motion, media-grid polish, haptics) and other bonuses (FCM, voice notes, instrumentation/UI tests) are not started.
